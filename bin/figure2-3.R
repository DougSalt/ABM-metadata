...
if(length(args) < 3) {
  print(paste("Usage: figure2-3K4.R [-showdata] [-plotlm] [-plotgam4] [-bs] <data> ",
              "<y-axis> <PDF>"),
        quote = FALSE)
  q(status = 1)
}
...
pdf(pdf.file, paper = "a4", width = 7, height = 9)
...
for(j in 1:length(scenarios)) {
  subdata <- subset(data, Scenario == scenarios[j])
  ...
  plot(0, type = "n", bty = "n", xlim = c(0, max(Incentive)),
       ...
       main = paste(scenarios[j], " (", dev.expl, ")", sep = ""))
